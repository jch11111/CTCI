...
var MakeGrid = function (r, c) {
    ...
    this.setRowCol0 = function () {
        //reverse returns a closure with access to grid and ux
        return function () {
            grid.ZeroRows = new Array();
            grid.ZeroCols = new Array();
            grid.forEachNode(
                function (node) {
                    if (node.val === 0) {
                        if (grid.ZeroRows.indexOf(node.x) < 0) grid.ZeroRows.push(node.x);  //add the current row to the zero row cache
                        if (grid.ZeroCols.indexOf(node.y) < 0) grid.ZeroCols.push(node.y);  //add the current col to the zero col cache
                    }
                }
            );
            grid.forEachNode(
                function (node) {
                    //for each node, if it's x is in ZeroRows, or y is in ZeroCols it is zero, else it is 1
                    node.val = 1;
                    if (grid.ZeroRows.indexOf(node.x) >= 0 || grid.ZeroCols.indexOf(node.y) >= 0) {
                        node.val = 0;
                    }
                }
            );
...
$('#reverse').on('click', mg.setRowCol0());
